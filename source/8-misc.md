...
{% note danger 警告 2 %}
可以启用快速文件记录，这需要完全兼容的固件 FAT32 驱动程序。在不合适的 FAT32 写入支持的驱动程序上（例如：APTIO IV，可能还有其他），此设置可能会导致损坏，包括无法使用 ESP 文件系统，因此，如果要测试此选项，请准备重新创建 ESP 分区及其所有的内容。此选项可以在某些合适的固件上显着地提高记录速度，但在其他一些固件上可能几乎没有速度差异。
{% endnote %}
