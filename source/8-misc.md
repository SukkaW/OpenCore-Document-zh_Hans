...
引入一个延迟可以提供额外的时间来保持正确的 `Action Hotkey` 序列，例如，启动到恢复模式。在大多数系统中，初始启动标志的出现是一个很好的指示，表明可以按住热键的时间。比这更早的时间，按键可能不会被注册。在一些平台上，由于键盘驱动程序的性质，要访问动作热键，还需要将该选项设置为至少5000-10000微秒。

如果配置了开机报时（见音频配置选项），那么以较慢的启动速度为代价，可以使用半秒到一秒的更长的延迟（500000-1000000）来创造类似于真正的Mac的行为，其中报时本身可以作为热键可以被按下的一个信号。在OpenCore中，由于必须首先加载和连接非本地驱动程序，因此开机鸣叫在开机顺序中不可避免地比在苹果硬件上晚。配置开机鸣叫并增加这个较长的额外延迟，对于那些开机时间快或显示器信号同步慢的系统可能导致在某些开机或重启时根本不显示开机标识的情况也很有用。
...
UEFI 变量日志不包括一些信息，没有性能数据。为了保持系统的完整性。日志的大小被限制为 32KB。一些类型的固件可能会更早地截断它，或者在没有内存的情况下完全放弃。使用非易失性标志将导致日志在每打印一行后被写入 NVRAM 闪存。
...
{% note danger 警告 2 %}
启用快速文件记录是可能的，这需要一个完全兼容的固件 FAT32 驱动程序。在不正确的 FAT32 写入支持的驱动程序上（例如APTIO IV，但也可能是其他的），这个设置可能会导致损坏，甚至包括无法使用的 ESP 文件系统，因此，如果测试这个选项，要准备重新创建 ESP 分区及其所有的内容。在一些合适的固件上，这个选项可以大大增加记录速度，但在其他一些固件上可能没有什么速度差异。

在解释日志时，请注意这些行的前缀是描述日志行的相关位置（模块）的标签。
的相关位置（模块）的标签，以便更好地将该行归入功能。
