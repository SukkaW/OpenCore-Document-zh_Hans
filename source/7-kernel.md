...
与 macOS 10.7 不同（在该版本中，某些主板标识被视为仅支持 i386 架构的设备），以及 macOS 10.5 或更早版本（在这些版本中，macOS 内核不支持 x86_64 架构），macOS 10.6 非常特殊。在 macOS 10.6 上的架构选择取决于许多因素，包括不仅是主板标识，还包括 macOS 产品类型（客户端 vs 服务器）、macOS 版本号和内存量。检测所有这些因素是复杂且不切实际的，因为几个版本存在实现缺陷，导致无法正确执行服务器检测。因此，当设置为 Auto 时，OpenCore 在 macOS 10.6 上会在支持的情况下回退到 x86_64 架构，就像在 macOS 10.7 上一样。但是，32 位 KernelArch 选项仍然可以显式配置。
